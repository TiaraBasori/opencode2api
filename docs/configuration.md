...
- 仅当请求 **未传入** `tools` 且 `OPENCODE_INTERNAL_WEB_FETCH_ENABLED=true` 时生效。
- 该模式不会启用外部工具桥接协议，而是直接放行 OpenCode 原生 `web_fetch`。
- 代理会读取后端工具列表，并仅允许名为 `web_fetch`（或以 `.web_fetch` / `/web_fetch` 结尾）的内置工具。
- 一旦客户端传入 `tools`，请求立即切回外部工具桥接模式，所有 OpenCode 内置工具继续保持禁用。
- 如果后端未提供 `web_fetch`，代理会自动回退到“全部内置工具禁用”的安全模式。
